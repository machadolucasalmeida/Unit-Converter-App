...
fun MainScreenUnitCategory(navController: NavController, viewModel:HomeViewModel, modifier: Modifier = Modifier.background(MaterialTheme.colorScheme.background)) {
    ...
    var wasFavIconClicked by remember { mutableStateOf(false) }
            ...
            Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Card(modifier = Modifier
                        ...
                        onClick = {
                            viewModel.infoRepository.getInfo().unitCategory = listOf(
                                "Kilometer", "Meter", "Centimeter",
                                "Millimiter", "Feet")
                            viewModel.infoRepository.getInfo().titleCategory = "Length"
                            viewModel.infoRepository.getInfo().unitAbreviation = listOf(
                                "km", "m", "cm", "mm", "ft")
                            //navController.navigate("UnitCalculator")
                            wasFavIconClicked = true
    ...
    if(wasFavIconClicked){
        AlertDialog(onDismissRequest = {wasFavIconClicked = false}) {
            Text(text = viewModel.infoRepository.getInfo().titleCategory, color = MaterialTheme.colorScheme.surface)
        }
    }
}
